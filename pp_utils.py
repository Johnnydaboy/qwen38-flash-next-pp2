...
def compute_need_sampled_mask(input_batch: InputBatch) -> np.ndarray | None:
    ...
    # FIX 54436: do not infer completion before sampling. With speculative
    # decoding, num_computed_tokens may overrun the nominal output boundary
    # while the request is still active. Dropping that row freezes sampled
    # token updates on earlier PP ranks and produces repeating output.
